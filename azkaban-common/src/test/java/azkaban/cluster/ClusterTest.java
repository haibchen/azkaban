...
public class ClusterTest {
  ...
  public void testClusterUrlsWithLibraryPathOfJars() throws IOException {
    ...
    Cluster cluster = new Cluster("default", clusterProps);

    List<URL> clusterUrls = cluster.getClusterComponentURLs(Arrays.asList("hive", "hadoop"));
    Set<String> clusterUrlFiles = new HashSet<>();
    for (URL url : clusterUrls) {
  ...
  public void testClusterUrlsOfSubsetOfComponentsWithLibraryPathOfWildcards()
    ...
    List<URL> clusterUrls = cluster.getClusterComponentURLs(Arrays.asList("hive"));
    Set<String> clusterUrlFiles = new HashSet<>();
    for (URL url : clusterUrls) {
      clusterUrlFiles.add(url.getFile());
    }
  ...
  public void testClusterUrlsOfNonexistentComponentsWithLibraryPathOfWildcards()
    ...
    List<URL> clusterUrls = cluster.getClusterComponentURLs(Arrays.asList("pig"));
  }

  private File makeTestJar(String folderName, String jarName) throws IOException {
    File folder = testDir.newFolder(folderName);
  ...
  public void testJavaLibraryPathAsString() throws IOException {
    ...
    String[] libraryPaths = cluster.getJavaLibraryPath(Arrays.asList("hadoop", "hive"))
        .split(Cluster.PATH_DELIMITER);
    Set<String> javaLibraryPaths = new HashSet<>(Arrays.asList(libraryPaths));
  ...
  public void testNativeLibraryPathAsString() throws IOException {
    ...
    String[] libraryPaths = cluster.getNativeLibraryPath(Arrays.asList("hadoop", "hive"))
        .split(Cluster.PATH_DELIMITER);
    Set<String> javaLibraryPaths = new HashSet<>(Arrays.asList(libraryPaths));
